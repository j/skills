...
Models: **gpt-5.5** at `--effort xhigh` (read-only run per [codex.md](codex.md)) or **opus-4.8** at xhigh effort (Agent tool, `subagent_type: "claude"`, `model: "opus"`). xhigh buys depth, but depth can wander: if the diagnostician sprawls — chasing side quests, piling up theories, making a mess of its report — disregard its work entirely and launch a fresh agent on **fable-5** at high effort with the same brief. Replace a wanderer; never iterate with one.
...
If this is a bug/debugging task: load the diagnosing-bugs skill first — `Skill(diagnosing-bugs)` / `$diagnosing-bugs` — if available, and follow it.
